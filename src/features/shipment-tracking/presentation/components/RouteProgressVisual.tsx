...
export function RouteProgressVisual({ currentStatus, events }: RouteProgressVisualProps) {
    ...
    <div className="bg-white rounded-lg border shadow-sm p-6">
      ...
      <div className="space-y-4">
        {statusOrder.map((status, index) => {
          ...
          const isActive = isCurrent || (isDelayed && status === 'delayed') || (isDeliveryFailed && status === 'delivery_failed');
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <span className={`font-medium capitalize ${isActive ? 'text-blue-600' : isCompleted ? 'text-gray-900' : 'text-gray-400'}`}>
                    {SHIPMENT_STATUS_LABELS[status]}
                  </span>
                  {isActive && (
                    <span className="text-xs px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full">
                      {isDelayed && status === 'delayed' ? 'Delayed' : 
                       isDeliveryFailed && status === 'delivery_failed' ? 'Failed' : 
                       'Current'}
                    </span>
                  )}
                </div>
                <p className={`text-sm ${isCompleted ? 'text-green-600' : isActive ? 'text-blue-600' : 'text-gray-400'}`}>
                  {hasEvent && (
                    <>
                  ...
                  {!hasEvent && !isFuture && !isCurrent && <span>No update</span>}
                </p>
              </div>
            </motion.div>
          );
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-red-600">
                {events.filter(e => e.status === 'delayed').map(e => (
                  <span key={e.id} className="block">
        ...
        {isDeliveryFailed && (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-red-600">
                {events.filter(e => e.status === 'delivery_failed').map(e => (
                  <span key={e.id} className="block">
                    {e.description} - {new Date(e.timestamp).toLocaleString()}
                  </span>
                ))}
