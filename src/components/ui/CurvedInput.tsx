...
const edgeSeg = (
  ...
  // TypeScript narrowing - after straight check, R and dir exist
  const curved = g as { straight: false; R: number; dir: number };
  const { R, dir } = curved;
...
const bentRectPath = (
  ...
  const rc = Math.max(
    0,
    Math.min(radius, (vBot - vTop) / 2, (u1 - u0) / 2)
  );
  return [
    `M ${fmt(g, u0 + rc, vTop)}`,
...
export const CurvedInput = ({
  ...
  }, [geom, layout, caretIndex, display.length, btnTextW]);
    ...
    if (svg && geom && layout && textEl && display.length) {
      try {
        ...
        for (let i = 0; i <= display.length; i++) {
          const li =
            i === 0 ? 0 : textEl.getSubStringLength(0, i);
  ...
  return (
    ...
    >
      ...
      <input
        ...
        aria-label={ariaLabel || placeholder || 'Search tracking number or customer'}
        autoComplete="off"
        autoCapitalize="none"
        autoCorrect="off"
        spellCheck={false}
